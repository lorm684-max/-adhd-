(function(){
  function createTaskSystem({data, storage, ui, stateMachine, animation}){
    ...
    function setCustomTask(task){
      const level = data.levelOrder.includes(task.level) ? task.level : 'mushroom';
      currentLevelIndex = data.levelOrder.indexOf(level);
      currentTask = {
        level,
        text:task.text,
      ...
      ui.setTaskCard(currentTask, data.levelLabel[level], '杜包自己定的任务');
      stateMachine.setPetState('task', {say:false});
      animation.playAnimation('getTask', {force:true});
      ui.say('杜包自己写的任务，杜望已经叼住了。现在轮到你动一下。');
      ui.floater('📝');
      ui.openPanel('right');
    }
    ...
    return {newTask, setCustomTask, failTask, completeTask, getCurrentTask:() => currentTask};
  }
